...
def save_blocked_users(blocked_users):
    try:
        ...
        logger.error(f"Ошибка при сохранении списка заблокированных пользователей: {e}")

# Загружаем список заблокированных пользователей при старте
...
async def unblock_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if user_id in blocked_users:
            ...
            await update.message.reply_text(f"Пользователь {user_id} не был заблокирован.")
    except ValueError:
        await update.message.reply_text("Неверный ID пользователя. ID должен быть числом.")
